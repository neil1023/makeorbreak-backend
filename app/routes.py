...
def new_request():
    if request_format_okay(request):
        ...
        new_request = Request(title=data["request"]["title"], description=data["request"]["description"], geo=geo_string, image_encoded=data["image_url"], price=data["request"]["price"])
        print(new_request)
